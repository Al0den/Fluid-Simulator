...
void Engine::init_density_coeffs() {
    this->density_coeffs = _mDevice->newBuffer(sizeof(float) * SCREEN_WIDTH * SCREEN_HEIGHT, MTL::ResourceStorageModeShared);
    ...
    this->density_coeffs->didModifyRange(NS::Range::Make(0, sizeof(float) * SCREEN_WIDTH * SCREEN_HEIGHT));
}
...
void Engine::init_gradient_buffers() {
    this->gradientX = _mDevice->newBuffer(sizeof(float) * SCREEN_WIDTH * SCREEN_HEIGHT, MTL::ResourceStorageModeShared);
    this->gradientY = _mDevice->newBuffer(sizeof(float) * SCREEN_WIDTH * SCREEN_HEIGHT, MTL::ResourceStorageModeShared);
...
void Engine::init_density_map() {
    this->density_map = _mDevice->newBuffer(sizeof(float) * SCREEN_WIDTH * SCREEN_HEIGHT, MTL::ResourceStorageModeShared);
